...
describe('LocationRoomWatchPage', () => {
  ...
  it('renders alias freshness, map link, readable transcript identity, sidebar stats, and structured rolls', () => {
    ...
    expect(screen.queryByLabelText('Adventure signals')).not.toBeInTheDocument();
    expect(screen.queryByText('Stakes')).not.toBeInTheDocument();
    expect(screen.queryByText('If no one answers the bell, the buried choir will wake hungry.')).not.toBeInTheDocument();
    expect(screen.queryByText('Decision options')).not.toBeInTheDocument();
    expect(screen.queryByText('How will the room answer the buried bell?')).not.toBeInTheDocument();
    expect(screen.queryByText('Muffle the bell')).not.toBeInTheDocument();
    expect(screen.queryByText('Follow the ringing')).not.toBeInTheDocument();
    expect(screen.queryByText('Declared action')).not.toBeInTheDocument();
    expect(screen.queryByText('Wagdie #7 wraps the bell in a torn cloak to choke its ringing.')).not.toBeInTheDocument();
    expect(screen.queryByText('Choice: Muffle the bell')).not.toBeInTheDocument();
    expect(screen.queryByText('Intent: Sabotage')).not.toBeInTheDocument();
  ...
  it('renders narrative scene-check labels and scene-specific structured roll wording', () => {
    ...
    expect(screen.queryByText('The safe path is clear, but the ash sloughs toward the stairwell.')).not.toBeInTheDocument();
    expect(screen.queryByText('Complication · Partial Success')).not.toBeInTheDocument();
    expect(screen.queryByText('Clock pressure')).not.toBeInTheDocument();
    expect(screen.queryByText('Ash Collapse')).not.toBeInTheDocument();
    expect(screen.queryByText('2 / 6')).not.toBeInTheDocument();
    expect(screen.queryByText('The ruin is closer to sealing the route behind them.')).not.toBeInTheDocument();
