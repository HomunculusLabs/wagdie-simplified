...
describe('game-master beat generator helpers', () => {
  ...
  it('repairs a collected invalid model response once and returns safe diagnostics', async () => {
    ...
    expect(repairPrompt).toContain('Failure detail: Game-master beat response did not contain a JSON object')
    ...
    expect(repairPrompt).toContain('Opening publicNarration must be at least 280 characters and 3-5 complete sentences')
    expect(repairPrompt).toContain('publicNarration must include an actionable choice/interaction term')
    expect(repairPrompt).toContain('Avoid passive atmosphere-only narration')
    expect(repairPrompt).toContain('Non-aftermath beats require currentObjective')
    expect(repairPrompt).toContain('"adventurePatch"')
    expect(repairPrompt).toContain('Write at least 280 characters across three complete sentences')
    expect(repairPrompt).toContain('A concrete door, stair, route, mark, or other location feature changes in sentence one')
    expect(repairPrompt).toContain('inspect, open, follow, search, approach, or retreat')
    expect(repairPrompt).toContain('"ttrpgPhase":"exploration"')
    expect(repairPrompt).toContain('"combatReadiness":"foreshadow"')
    expect(repairPrompt).toContain('"threatLevel":1')
  ...
  it('repairs duplicate recent GM outcome openings instead of returning static fallback', async () => {
    ...
    expect(repairPrompt).toContain('Return exactly one JSON object: first character { and last character }')
    expect(repairPrompt).toContain('Use one exact enum string per enum field; never output pipe-separated choice lists')
    expect(repairPrompt).toContain('Sentence one ties Ash')
    expect(repairPrompt).toContain('at least two concrete options')
    expect(repairPrompt).toContain('"decision":"danger"')
    expect(repairPrompt).toContain('"threatLevel":1')
